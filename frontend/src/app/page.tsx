...
import { Search, Activity, ArrowLeft, CheckCircle2, User, Bot, TrendingUp, ShieldCheck } from 'lucide-react';
  ...
  const RenderChart = ({ chartData }: { chartData?: any }) => {
    ...
    return (
      <div className="w-full flex flex-col items-center mt-6 p-4 bg-slate-50 rounded-xl">
        <div className="text-[10px] font-black text-slate-400 mb-4 self-start">营收与利润趋势 (2023-2025)</div>
  ...
  return (
    <div className="min-h-screen bg-[#FDFDFF] flex flex-col font-sans">
      <main className="flex-1 flex flex-col items-center relative overflow-hidden">
        {stage === 'home' ? (
          <div className="w-full max-w-4xl pt-40 px-6 flex flex-col items-center animate-in fade-in slide-in-from-bottom-4">
            ...
            <div className="w-full relative max-w-2xl">
              ...
              />
              <button onClick={() => handleSend()} className="absolute right-3 top-3 bottom-3 px-6 bg-violet-600 text-white font-bold rounded-xl hover:bg-violet-700 transition-colors">启动审计</button>
            </div>
          </div>
        ) : (
          <div className="w-full max-w-5xl h-full flex flex-col pt-10 pb-20 px-6 overflow-y-auto">
             <div className="flex justify-between items-center mb-12">
                <button onClick={() => setStage('home')} className="flex items-center text-slate-400 font-bold hover:text-slate-600 transition-colors">
            ...
            <div className="space-y-8">
                                ...
                                <div className="space-y-4">
                                    {msg.metrics ? (
                                        <div className="animate-in zoom-in-95 duration-500">
                                            ...
                                            <div className="grid grid-cols-2 gap-4 mb-6">
                                                <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                                                    ...
                                                    <div className="text-2xl font-black text-violet-600">{msg.metrics.health?.overall || 90}</div>
                                                </div>
                                                <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                                                    <div className="text-[10px] font-black text-slate-400 uppercase mb-1">最新营收</div>
                                                    <div className="text-2xl font-black text-slate-900">{msg.metrics.health?.latest_revenue || "已核定"}</div>
                                                </div>
                                            </div>
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            {msg.loading && <div className="text-xs text-violet-500 font-bold animate-pulse">Agent 正在勾稽底层账目...</div>}
                                        </div>
                                    )}
                                </div>
                            ) : (
                ...
                {error && <div className="p-4 bg-red-50 text-red-500 rounded-xl text-xs font-bold border border-red-100 text-center">{error}</div>}
